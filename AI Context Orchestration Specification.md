...
- **当 `supportToolCall == false` 时（文本注入 fallback 方案）**：
  1. `QuickRepliesTool` 必须通过 `prepareFallbackMessages` 注入格式化规范，并通过
     `parseFallback` 提取 XML 标签；`ChatContextBuilder` 不得包含工具专属逻辑。
...
- Provider 无关请求可通过 `AiResponseFormat` 描述结构化响应名称、JSON Schema 和
  strict 模式。`OpenAiCompatibleProvider` 必须映射为
...
1. **提取消息正文与快捷回复（Suggestions）**：
   ...
   - **不支持工具调用的模型**：由 `QuickRepliesTool` 读取助手消息文本，通过
     `<suggestions>([\s\S]*?)</suggestions>` 提取最多 4 个选项，并从正文中移除标签块。
2. 对内容执行首尾空白清理。
