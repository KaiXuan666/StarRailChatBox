...
自动总结默认在未压缩有效消息达到 20 条时触发，并保留最近 8 条原始消息。总结请求
...
1. **多模态模型切换**：
   - 若当前待发送的用户消息中包含图片附件（包括相册选择、拍照等），编排器 MUST 切换使用多模态模型配置（`id = "multimodal"`，其 `supportVision` 为 `true`）；不包含图片时默认使用默认普通模型配置（`id = "default"`）。
2. **当前消息结构构造**：
   - 在多模态模式下，当前用户消息在 CHAT API 中映射为多段内容结构（在 OpenAI 协议中表现为 `parts` 数组，即 `List<AiContentPart>`）。
   - 文本部分封装为 `AiContentPart.Text`。
   - 图片部分必须通过跨平台读取服务转换为 Base64 字符串，并封装为 `AiContentPart.ImageUrl`（其值为 `data:<mime>;base64,<data>` 的 Data URL 格式）。
   - Web 端若直接使用 `data:` URI 作为文件标识，底层可跳过字节读取，直接在 ContentPart 中使用。
3. **文本文件附件的处理与持久化**：
   - 考虑到非图片类型的普通文本文件（如代码文件、文本日志等）可能会非常大，且不适合直接作为多模态图片传给模型，系统必须读取其内容为文本字符串。
   - 在将消息写入数据库持久化和发送给模型前，将文本文件内容按如下约定格式拼接到用户提示词 Prompt 的尾部：
     ```text
     [文件名.扩展名]
     ---
     <文件文本内容>
     ---
     ```
   - 拼接后的完整文本作为 `chat_message` 的正文存入 Room/浏览器数据库。这保证了在以后重新加载会话历史或进行历史裁剪重播时，该附件内容已作为历史文本消息固化在上下文中，不会因为原始物理文件不可用或未重复读取而丢失。
4. **历史消息的多模态重播边界**：
   - 由于数据库表结构目前未针对多模态进行列扩展（没有专门存储图片 BLOB 或图片列表），会话历史在重新加载时，之前的历史消息在上下文构造中均作为纯文本消息载入，即不重播历史图片附件。图片附件仅在用户首次发送当前输入时，作为 `AiContentPart.ImageUrl` 传输给大模型。
...
- `ToolCallCoordinator` 按响应顺序执行工具；`TerminalOutput` 直接形成最终助手消息，
  `Executable` 生成 `tool` 消息并继续请求模型。
...
### 7.4 多模态 API 与模型配置（Multimodal API Settings）

- 除了默认模型配置外，系统增加了独立的多模态模型配置（`id = "multimodal"`）。
- 在多模态 API 设置中保存的模型配置，其 `supportVision` 属性必须显式设置为 `true`。
- 此配置的加载与保存通过 `ModelConfigRepository.getMultimodal` 和 `saveMultimodal` 方法实现，能够独立存在并保持与普通默认模型配置的状态和持久化数据隔离。
...
| 1.14 | 2026-06-08 | 开启多模态与附件选择支持，细化多模态上下文构建、UriReader跨平台字节读取与文本文件拼接持久化机制 |
