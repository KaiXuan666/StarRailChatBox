...
分支会话通过 `chat_session_segment` 保存“当前会话可见的历史片段路径”。分支创建时
只写入会话元数据和片段引用，不复制历史 `chat_message` 或附件记录。分支上下文按片段
顺序读取：先读取继承的分叉点前历史，再读取分支自身新增消息。多级分支继续复制并截断
片段路径，形成稳定的可见历史。重新生成等当前会话局部隐藏行为通过
`chat_session_hidden_message` 按 owner session 记录；查询某个会话的展示、快捷建议、
上下文和摘要来源时，只排除该 owner session 隐藏的消息，不影响其他分支已继承的历史
快照。

消息展示列表 MUST 使用独立的本地分页数据流，按 `seq DESC` 从最新消息向更早消息
加载。默认首次加载 50 条、每页 50 条、预取 10 条且不启用占位符。分页窗口和页面
回收只影响 UI 展示，不得改变 AI 上下文查询、摘要来源、消息持久化或 `seq` 顺序。
跳转到会话最早消息时 MUST 直接以最早页 offset 创建新的分页 generation，只加载
最早 50 条，不得依次加载全部中间页面；返回最新消息时同理从 offset 0 重新加载。
执行最早或最新消息跳转前 MUST 先检查对应分页边界是否已在当前缓存窗口中；已缓存时
直接无动画滚动，不得替换分页 generation。点击快捷回复时 MUST 立即无动画返回最新
消息；仅当最新边界不在缓存中时才重新创建最新页 generation。
...
名称、头像 URI、创建时间和最近会话时间，不包含 prompt、开场白、模型参数、语音样本
或编辑草稿。

Room 实现 MUST 使用 DAO 投影直接返回摘要字段，禁止先查询完整 `agent_role` 再映射。
聊天 UI 状态不得持有完整角色。只有展示临时开场白、发送或重试消息、角色编辑或导出
等确实需要完整字段的操作，才允许通过 `CharacterRepository.getCharacter(id)` 临时
加载单个完整角色；操作完成后不得把完整角色写入常驻聊天 `UiState`。
...
4. 找到会话后，以 `seq DESC` 分页读取消息，并通过反向列表按时间顺序展示；上滑时
   继续加载更早消息。
5. 未找到会话时，按角色 ID 临时读取完整角色的 `openingMessage`；非空时只在 UI
   显示该欢迎消息，完整角色不得进入聊天 `UiState`。
...
5. 从 assistant 成功消息开启分支时，立即创建并切换到新会话；新会话展示截至目标
   assistant 消息的共享历史，后续发送只写入分支自己的开放片段。
6. 分支会话可使用独立默认标题，但仍 MUST 进入自动命名流程；当分支自己的开放片段中
   至少完成一轮 user/assistant 对话后，若标题仍是默认占位标题，应自动生成会话标题。
...
用户只能对当前会话最新一条 `completed` assistant 消息触发重新生成。执行时 MUST：

1. 在持久化层复查目标消息仍是当前会话可见路径中最新的 assistant 成功消息。
2. 只在当前会话写入局部隐藏记录，不得更新目标 `chat_message.deleted_at`。
3. 将会话最后消息信息回退到当前会话可见路径中的上一条消息。
4. 不新增或改写 user 消息，不改变上一条用户输入的 `seq`、正文或附件。
5. 使用隐藏后上下文中的最后一条 user 消息作为当前输入重新构造 CHAT 请求。
6. 被当前会话隐藏的旧 assistant 回复不得进入当前会话重新生成请求的历史上下文。
7. 已存在分支继续按各自 owner session 的隐藏记录读取历史；父会话重新生成不得破坏
   分支创建时的历史快照。
8. 重新生成失败时，按普通 CHAT 失败规则追加 failed assistant 记录，保留用户消息。
...
分支上下文可能包含多个来源 session，不能用会话内 `seq` 在不同来源之间比较来判断
当前输入之前的历史。发送流程 MUST 按仓库返回的可见上下文顺序，取最后一条 user 消息
作为当前输入，并把它之前的消息作为历史。
...
消息，并且只能局部隐藏当前会话的旧回复，不得全局删除或隐藏其他分支继承的消息。

未来若允许并发生成，必须先引入明确的请求 ID 或并发分支模型，不得继续假设“会话内
永远只有一个进行中的请求”。
...
聊天主界面的角色选择列表依赖 `CharacterRepository.loadCharacterSummaries()`；角色切换
和消息展示只使用摘要及角色 ID。发送、重试或展示临时开场白时才按 ID 临时调用
`CharacterRepository.getCharacter(id)`。UI 状态不得持有完整角色。
...
- 聊天主界面只保存 `CharacterSummary` 列表和角色 ID，不因角色切换加载完整角色。
- `ChatUiState` 不持有完整角色，发送与重试按角色 ID 临时查询并绑定点击时的会话 ID。
- 不同角色的草稿、发送状态和后台请求彼此独立。
- 聊天展示首次只加载最新 50 条，上滑可连续加载更早消息且无重复或乱序。
- 失败 assistant 记录跨分页边界时仍不直接展示，并正确标记对应 user 消息失败。
- 展示分页不会改变 AI 上下文、摘要来源和当前输入去重规则。
...
| 1.21 | 2026-06-12 | 聊天状态移除完整角色对象，仅在欢迎语、发送和重试时按角色 ID 临时读取 |
| 1.22 | 2026-07-08 | 增加最新 assistant 消息重新生成规则，删除旧回复后复用最后一条 user 消息请求 |
