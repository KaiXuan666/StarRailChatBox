...
1. 在持久化层复查目标消息仍是当前会话未删除消息中 `seq` 最大的 assistant 成功消息。
2. 删除该 assistant 消息，并将会话最后消息信息回退到上一条未删除消息。
3. 不新增或改写 user 消息，不改变上一条用户输入的 `seq`、正文或附件。
4. 使用删除后上下文中的最后一条 user 消息作为当前输入重新构造 CHAT 请求。
5. 被删除的旧 assistant 回复不得进入重新生成请求的历史上下文。
6. 重新生成失败时，按普通 CHAT 失败规则追加 failed assistant 记录，保留用户消息。
